...
check_alps_uenv() {
    if [[ ! -d "/user-environment/linux-neoverse_v2" ]]; then
        echo "Please pull the uenv image: 'uenv image pull prgenv-gnu/26.3:v1'"
        echo "Then, start a new session with that image: 'uenv start --view=default prgenv-gnu/26.3:v1'"
        exit 2
    fi
    # if [[ ! -d "/user-environment/linux-neoverse_v2" ]]; then
